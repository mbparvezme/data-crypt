...
export class DataCrypt {
    static async deriveKey(password, salt, { iterations = 100000, hash = 'SHA-256', length = 256 } = {}) {
        const keyMaterial = await subtle.importKey('raw', enc.encode(password), 'PBKDF2', false, ['deriveKey']);
        return subtle.deriveKey({ name: 'PBKDF2', salt, iterations, hash }, keyMaterial, { name: 'AES-GCM', length }, false, ['encrypt', 'decrypt']);
    }
