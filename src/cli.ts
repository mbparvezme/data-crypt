...
function parseArgs(): CLIOptions {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (arg) {
      ...
      // New Flags
      case '-z': case '--compress': options.compress = true; break;
...
async function main() {
  ...
  try {
    ...
    const cryptoOpts: DeriveOptions = {
    ...
    if (options.file) {
      if (!input) { console.error('Error: Input file path required with -f'); exit(1); }
      ...
      // Text Operations
      if (!input) { console.error('Error: No input text'); exit(1); }
      ...
      if (options.operation === 'encrypt') {
        ...
        if (options.html) {
            ...
            if (options.output) writeFileSync(options.output, htmlContent);
            else console.log(htmlContent);
        } else {
            if (options.output) writeFileSync(options.output, encrypted);
